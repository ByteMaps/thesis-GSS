...
# Create network for visualization
    ...
    plt.title(figtitle)
    if savef:
        plt.savefig('fig_prob/' + simstr + figtitle + '.png')
...
nrparamchange=30
probs = np.linspace(0,0.2,nrparamchange)
...
for k in range(nrparamchange):
    prob_createlink = probs[k]
    prob_removelink = probs[k]
    for l in range(sims):
        print(k,l)
...
# np.save('out_probs_0_02_nrparams_30_2_OPs_stabil200',OPs)
# np.save('out_probs_0_02_nrparams_30_2_categories_stabil200',categories)
# np.save('out_probs_0_02_nrparams_30_dist_opinions_stabil200',dist_opinions)
# np.save('out_probs_0_02_nrparams_30_probs_stabil200',probs)
