...
def form_netw_chart(modeltype, model, N, category, opinions, cmap, G2, pos, path, savefigs, showfigs):
...
def	visualise_network(modeltype, model, N, opinions, link_matrix, category, path, savefigs, showfigs):
	"""Create a network plot based on a single model run"""
	edges = form_edges(N, link_matrix)
	pos, cmap, G2 = form_network(N, edges)

	form_netw_chart(modeltype, model, N, category, opinions, cmap, G2, pos, path, savefigs, showfigs)

# ====================== OP_DIST GRAPH ======================================

def	form_density_estimate(modeltype, opinions, model, path, savefigs, showfigs):
