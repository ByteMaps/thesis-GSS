...
# Create network for visualization
    ...
    plt.title(figtitle)
    if savef:
        plt.savefig('figs_distremovelink/' + simstr + figtitle + '.png')
...
nrparamchange=30
dist_removelinks = np.linspace(0.0,0.3,nrparamchange)
...
for k in range(nrparamchange):
    dist_removelink= dist_removelinks[k]
    for l in range(sims):
        print(k,l)
...
# np.save('out_dist_removelink_0_03_nrparams_30_OPs',OPs)
# np.save('out_distremovelink_0_03_nrparams_30_categories',categories)
# np.save('out_T_01_15_nrparams_30_dist_opinions_stabil200',dist_opinions)
# np.save('out_dist_removelink_1_15_nrparams_15_distremovelinks',dist_removelinks)
