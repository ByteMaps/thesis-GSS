...
# Create network for visualization
    ...
    plt.title(figtitle)
    if savef:
        plt.savefig('fig_tries_opchange/' + simstr + figtitle + '.png')
...
tries_opinionchanges=np.concatenate(([1], np.arange(15,451,15)))
nrparamchange=len(tries_opinionchanges)

OPs=np.zeros((sims,N,nrparamchange))
categories=np.zeros((sims,nrparamchange))

for k in range(nrparamchange):
    tries_opinionchange = tries_opinionchanges[k]
    for l in range(sims):
        print(k,l)
...
# np.save('out_tries_opinionchange_1_450_nrparams_30_OPs_stabil200',OPs)
# np.save('out_tries_opinionchange_1_450_nrparams_30_categories_stabil200',categories)
# np.save('out_tries_opinionchange_1_450_nrparams_30_dist_opinions_stabil200',dist_opinions)
# np.save('out_tries_opinionchange_1_450_nrparams_30_tries_opinionchange_stabil200',tries_opinionchanges)
